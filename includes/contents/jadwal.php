<div id="content">
	...
	<?php
			...
			$jadwal = mysql_query("SELECT s.nm_siswa,s.id_siswa,k.kelas FROM siswa s,kelas k WHERE k.id_siswa='$_SESSION[nama]' and password='$_SESSION[pass]'");
			$j = mysql_fetch_array($jadwal);
			echo "<table><tr><td>Nama <td>:</td></td><td >$j[nm_siswa]</td></tr>
			<tr><td>NIS <td>:</td></td><td>$j[id_siswa]</td></tr>
			<form method='post'>
			...
			if($kode != 0){
			$jadwal = mysql_query("SELECT distinct j.* FROM jadwal j,kelas k WHERE k.id_siswa='$_SESSION[nama]' and j.kd_kelas=$kode");
			$start=1;
			while($a = mysql_fetch_array($jadwal)){
			if($start%2==0){echo "<tr class=\"dark\">";}else{echo "<tr class=\"light\">";}
			...
			<th style=\"border:1px solid #000;\">Ruang</th>
			<th style=\"border:1px solid #000;\">Tahun Ajaran</th></tr>";
			$jadwal = mysql_query("SELECT distinct j.*, g.id_guru, k.kelas FROM jadwal j,kelas k,guru g WHERE g.id_guru='$_SESSION[nama]' and j.kd_kelas=k.kd_kelas");
			$start=1;
			while($a = mysql_fetch_array($jadwal)){
			if($start%2==0){echo "<tr class=\"dark\">";}else{echo "<tr class=\"light\">";}
